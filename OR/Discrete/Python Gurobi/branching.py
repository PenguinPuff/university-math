...
# ---------------------
# LP relaxation with fixed variables
# ---------------------
...
def manual_branch_and_bound_scp_with_tree(branching_rule="max_coverage"):
    ...
    while stack:
        ...
        if branching_rule == "naive":
            ...
            branch_var = min(fractional, key=lambda x: abs(0.5 - x[1]))[0]
        elif branching_rule == "max_coverage":
            uncovered = get_uncovered_elements(node.fixed_vars)
            def coverage_score(j):
                return len(subsets[j] & uncovered)
            branch_var = max(fractional, key=lambda x: coverage_score(x[0]))[0]
        else:
